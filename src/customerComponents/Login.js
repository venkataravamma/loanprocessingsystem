...
function handleSubmit (e) {
    ...
    if (role === "admin" ) {
			...
			setIsLoggedIn(1);
      const data = {
        // emailId: this.state.emilId,
        password: this.state.password,
        role: this.state.role
      };
      axios
        .post("https://localhost:8080/user/adduserasadmin", data)
      ...
      axios
        .post("https://localhost:8080/user/adduserasloanmanager", data)
        .then(res => 
        toast.success("Message sent successfully",{position: toast.POSITION.TOP_RIGHT})
        ).catch(err => console.log(err),toast.error("something went wrong",{position: toast.POSITION.TOP_RIGHT}));
    ...
    axios
      .post("https://localhost:8080/user/adduserascustomer", data)
      .then(res => console.log(res),
      toast.success("Message sent successfully",{position: toast.POSITION.TOP_RIGHT}))
    ...
    axios
      .post("https://localhost:8080/user/signin", data)
      .then(res => console.log(res),
      toast.success("Message sent successfully",{position: toast.POSITION.TOP_RIGHT}))
      .catch(err => console.log(err),toast.error("something went wrong",{position: toast.POSITION.TOP_RIGHT}));
          ...
          <Col
            ...
            <Form onSubmit={this.handleSubmit} className="post">
              {/* <Form.Group >
                ...
                <Form.Control type="email" placeholder="Enter email" id="emailId" value={emailId} onChange={(e) => setEmailId(e.target.value)}/>
              </Form.Group> */}
              ...
              {isLoggedin ==null && <Redirect to="/home" />}
            </Form>
          </Col>
        </Row>
      </Container>
